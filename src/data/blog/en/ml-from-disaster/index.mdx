...
# Replace the original intro with the new version
old_intro_start = "# Titanic - Machine Learning from Disaster"
new_mdx_content = updated_path.read_text()
split_parts = new_mdx_content.split(old_intro_start, maxsplit=1)
final_content = split_parts[0] + intro_and_dict + split_parts[1].split("---", 1)[-1]

# Save to new file
final_mdx_path = Path("/mnt/data/titanic_with_intro_and_dictionary.mdx")
final_mdx_path.write_text(final_content)
